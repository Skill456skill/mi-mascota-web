...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Añadir después de la función saveCartToLocalStorage
  ...
  // Añadir después de la declaración de currentProductId
  let currentProductQuantity = 1
  let currentProductUnitPrice = 0 // Nuevo: para almacenar el precio unitario
  ...
  const productModalUnitPrice = document.getElementById("productModalUnitPrice") // Nuevo elemento para precio unitario
  const productModalSubtotal = document.getElementById("productModalSubtotal") // Nuevo elemento para subtotal
  ...
  // Modificar la función updateFeaturedProduct para que maneje los 3 productos más añadidos al carrito

  // Función para actualizar el producto destacado
  function updateFeaturedProduct() {
    // Crear una copia de los productos ordenados por addCount (de mayor a menor)
    const sortedProducts = [...products].sort((a, b) => b.addCount - a.addCount)

    // Tomar los 3 productos más añadidos (si tienen addCount > 0)
    const topProducts = sortedProducts.filter((product) => product.addCount > 0).slice(0, 3)

    // Si no hay productos añadidos, mantener el destacado predeterminado
    if (topProducts.length === 0) {
      const initialFeaturedProduct = products.find((p) => p.id === featuredProductId)

      if (initialFeaturedProduct) {
        if (featuredProductImage) featuredProductImage.src = initialFeaturedProduct.image
        if (featuredProductTitle) featuredProductTitle.textContent = initialFeaturedProduct.name
        if (featuredProductDescription) featuredProductDescription.textContent = initialFeaturedProduct.description

        if (initialFeaturedProduct.originalPrice) {
          if (featuredProductOriginalPrice) {
            featuredProductOriginalPrice.textContent = `${formatPrice(initialFeaturedProduct.originalPrice)}`
        ...
        if (featuredProductCurrentPrice)
          featuredProductCurrentPrice.textContent = `${formatPrice(initialFeaturedProduct.price)}`
      }

      return
    }

    // Actualizar el producto destacado actual con el más añadido
    featuredProductId = topProducts[0].id

    // Actualizar la información del producto destacado en la página
    if (featuredProductImage) featuredProductImage.src = topProducts[0].image
    if (featuredProductTitle) featuredProductTitle.textContent = topProducts[0].name
    if (featuredProductDescription) featuredProductDescription.textContent = topProducts[0].description

    if (topProducts[0].originalPrice) {
      if (featuredProductOriginalPrice) {
        featuredProductOriginalPrice.textContent = `${formatPrice(topProducts[0].originalPrice)}`
        featuredProductOriginalPrice.style.display = "inline"
      }
    } else {
      if (featuredProductOriginalPrice) featuredProductOriginalPrice.style.display = "none"
    ...
    if (featuredProductCurrentPrice) featuredProductCurrentPrice.textContent = `${formatPrice(topProducts[0].price)}`
  }

  // Añadir esta función después de la función updateCart()
  ...
  // Añadir este código después del bloque anterior
  // Capturar todos los botones de checkout por clase en lugar de por ID
  ...
  function showProductModal(productId) {
    ...
    if (product && productModal) {
      ...
      // Calc  productModalSubtotal.textContent = `$${formatPrice(product.price)}`
  ...
  // Añadir después de la función closeProductModal
  ...
  // Modificar la función proceedToCheckout para usar la cantidad seleccionada
  ...
  // Añadir eventos para los botones de cantidad
  ...
  // Añadir eventos para validación de campos
  formInputs.forEach((input) => {
  ...
  // Inicializar el producto destacado
  const initialFeaturedProduct = products.find((p) => p.id === featuredProductId)
  if (initialFeaturedProduct) {
    if (featuredProductImage) featuredProductImage.src = initialFeaturedProduct.image
    if (featuredProductTitle) featuredProductTitle.textContent = initialFeaturedProduct.name
...
// Esta función debe exportarse o llamarse desde la página de inicio (index.js)
function getTopProductsForHomepage() {
