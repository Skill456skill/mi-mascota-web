...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function checkUserAuthentication() {
    ...
    if (isLoggedIn) {
      ...
      // NUEVO: Mostrar carrito para usuarios logueados
      if (cartContainer) {
        cartContainer.style.display = "block"
        console.log("Carrito de compras mostrado para usuario logueado")
      } else {
      ...
      // NUEVO: Ocultar carrito para usuarios no logueados
      if (cartContainer) {
        cartContainer.style.display = "none"
        console.log("Carrito de compras ocultado para usuario no logueado")
      } else {
        console.log("Contenedor del carrito no encontrado")
      }
  ...
  // NUEVA FUNCIÓN: Verificar si el usuario está logueado antes de realizar acciones
  function checkLoginBeforeAction(action) {
    ...
    // Si está logueado, ejecutar la acción
  ...
  // NUEVA FUNCIÓN: Mostrar modal de login requerido
  ...
  // NUEVA FUNCIÓN: Cerrar modal de login requerido
  ...
  // NUEVOS EVENTOS: Manejar modal de login requerido
  const closeLoginRequiredModalButton = document.getElementById("closeLoginRequiredModalButton")
  ...
  if (cancelLoginRequiredButton) {
    cancelLoginRequiredButton.addEventListener("click", closeLoginRequiredModal)
  }
  ...
  // MODIFICADO: Evento del botón de checkout con verificación de login
  if (checkoutButton) {
  ...
  // NUEVA FUNCIONALIDAD: Cargar y mostrar los 3 productos más baratos en oferta
  const featuredProductsGrid = document.getElementById("featuredProductsGrid")

  if (featuredProductsGrid) {
    // Base de datos de productos (misma que en tienda.js) - SOLO 3 PRODUCTOS EN OFERTA
    const products = [
      {
        id: 1,
        ...
        pet: "perro",
        brand: "premium",
        description: "Alimento balanceado de alta calidad para perros de todas las edades. Contiene todos los nutrientes esenciales para mantener a tu mascota saludable y feliz.",
        featured: true,
        sale: true,
        addCount: 0,
      },
      {
        ...
        pet: "gato",
        brand: "happy",
        description: "Juguete interactivo que estimula el instinto de caza de tu gato. Perfecto para mantener a tu felino activo y entretenido durante horas.",
        featured: false,
        sale: true,
        addCount: 0,
      },
      {
        ...
        pet: "perro",
        brand: "natural",
        description: "Champú hipoalergénico formulado especialmente para mascotas con piel sensible. Limpia profundamente sin causar irritaciones.",
        featured: false,
        sale: true,
        addCount: 0,
      }
    ...
    try {
      // Filtrar productos en oferta (que tienen originalPrice y sale = true)
      const productsOnSale = products.filter((product) => product.originalPrice && product.sale)

      // Ordenar productos por precio (de menor a mayor) y tomar los 3 más baratos
      ...
      if (cheapestProductsOnSale.length > 0) {
        ...
        // Agregar los 3 productos más baratos en oferta
        ...
        console.log("Productos en oferta cargados:", cheapestProductsOnSale.map(p => `${p.name} - $${formatPrice(p.price)}`))
      } else {
  ...
  window.checkLoginBeforeAction = checkLoginBeforeAction
